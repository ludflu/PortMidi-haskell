               ...
               PmInternal *midi, midi_macosxcm_type m)
{
    ...
    while (remaining_length > 0)
    {
        ...
        { /* output message using running status */
            ...
            MIDITimeStamp timestamp)
{
    ...
    if (m->packet == NULL)
    {
        ...
        if ((err = midi_write_flush(midi, timestamp)) != pmNoError)
            return err;
...
static void midi_get_host_error(PmInternal *midi, char *msg, unsigned int len)
{
    ...
    if (m)
    { /* make sure there is an open device to examine */
        if (m->error[0])
        {
...
PmError pm_macosxcm_init(void)
{
    ...
    for (i = 0; i < numInputs; i++)
    {
        ...
        pm_add_device("CoreMIDI", cm_get_full_endpoint_name(endpoint),
                      TRUE, (void *)INT2VOIDP(endpoint), &pm_macosx_in_dictionary);
    }
    ...
    for (i = 0; i < numOutputs; i++)
    {
        ...
        /* Register this device with PortMidi */
        pm_add_device("CoreMIDI", cm_get_full_endpoint_name(endpoint),
                      FALSE, (void *)INT2VOIDP(endpoint), &pm_macosx_out_dictionary);
    }
    return pmNoError;
